...
export function queryHasAttr(tuple: QueryTuple, attr: string) {
    for (const tag of tuple.tags) 
        if (tag.attr === attr)
            return true;
    return false;
...
export function withoutAttr(tuple: QueryTuple, attr: string): QueryTuple {
    return {
        t: 'tuple',
        verb: tuple.verb,
        tags: tuple.tags.filter(tag => tag.attr !== attr),
    }
}

export function withoutStar(tuple: QueryTuple): QueryTuple {
    return {
        t: 'tuple',
        verb: tuple.verb,
...
export function withVerb(tuple: QueryTuple, verb: string): QueryTuple {
    return {
        t: 'tuple',
        verb,
        tags: tuple.tags,
...
export function withAttrs(tuple: QueryTuple, attrs: string[]): QueryTuple {
    if (attrs.length === 0)
        return tuple;

    const tags = tuple.tags.slice();

    for (const attr of attrs) tags.push({ t: 'tag', attr, value: { t: 'no_value' }});
...
export function tupleHas(tuple: QueryTuple, attr: string) {
    for (const tag of tuple.tags)
        if (tag.attr === attr)
            return true;
    return false;
...
export function tupleHasStar(tuple: QueryTuple) {
    for (const tag of tuple.tags)
        return tag.specialAttr && tag.specialAttr.t === 'star';
    return false;
}

export function tupleHasValue(tuple: QueryTuple, attr: string) {
    for (const tag of tuple.tags)
        if (tag.attr === attr)
...
export function tupleGetValue(tuple: QueryTuple, attr: string): TaggedValue {
    for (const tag of tuple.tags)
        if (tag.attr === attr)
            return tag.value;
    return null;
...
export function tupleGetStringValue(tuple: QueryTuple, attr: string) {
