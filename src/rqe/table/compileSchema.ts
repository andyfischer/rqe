...
import { getWithAttr, hasWithAttr, getSingleValue, listWithAttr, listAll, each, count,
        ...
        preInsert, insert, sendToListeners, deleteWithAttr, deleteAll, replaceAll,
        listenToStream } from './RuntimeFunctions'
...
export function compileSchema<ItemType = any>(decl: SchemaDecl): Schema<Table<ItemType>> {
    ...
    function supportEach() {
        ...
        declareFunc('eachWithFilter', function *handler(table: Table, args) {
            if (args.length !== 1)
    ...
    for (const funcDecl of decl.funcs || []) {
        ...
        if (parsedFuncName === 'listAll') {
            declareFunc('listAll', (table: Table, args) => {
                return listAll(schema, 'listAll', table);
            });
            continue;
        }
        ...
        if (parsedFuncName === 'listenToStream') {
            schema.supportsStatusTable = true;
            supportDeleteAll();
            declareFunc('listenToStream', (table: Table, args) => {
                return listenToStream(table, args);
            });
            continue;
        }
